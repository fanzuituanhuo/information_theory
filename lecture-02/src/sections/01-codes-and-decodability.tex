...
\]
Since $\ell(x)=-\log_2 p(x)$ for every symbol here,
\[
...
\studentrecall{Here $\ell(x)=-\log_2p(x)$ for every $x$, so equality holds term by term.}
\ifshowanswers\else\workspace[2]\fi
\answerprose{The equality holds term by term: each length satisfies
$\ell(x)=-\log_2p(x)$. Multiplying by $p(x)$ and summing therefore gives
$L(C)=\Ent(X)$.}
\end{yourturn}
